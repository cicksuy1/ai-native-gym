...
A small, real change with a checkable outcome, on the practice sandbox (**`sandbox/`**):

> Add a `--version` flag to the notes CLI so that running `python -m sandbox --version` prints the
> version (from the existing `VERSION` in `cli.py`) and exits 0.
...
```
You:    [plan mode] In sandbox/, where does the CLI parse flags, and where's the
        version defined? Read cli.py. Don't change anything yet.
Claude: [reads cli.py — no edits]
        Flags are parsed in build_parser() with argparse subcommands; the version
        "0.1.0" already lives in VERSION at cli.py:10, but it's never wired to a
        --version flag yet.
```

You started in `plan` mode so exploration couldn't accidentally edit anything, and you pointed it at
*the right file* instead of the whole repo. That "right tokens, not the most" instinct is
**Module 2 (Context)**.

**② Decide: plan, or just act?** → *deep in Module 4 (Planning)*

This change fits in one sentence ("wire a `--version` flag to the existing `VERSION` and exit"). The
...
```
You:    Before you implement: the success check is `python -m sandbox --version`
        prints the VERSION value and exits 0. Add a tiny test for that, run it,
        watch it fail, then implement until it passes.
Claude: [writes a test, runs it] Fails: no --version handling. ✅ red, as expected.
        [implements the flag, reruns] Passes: prints 0.1.0, exit 0. ✅
```
...
Claude: [guesses the flag-parsing spot, hardcodes "1.0.0" instead of using the
         existing VERSION, edits two files, auto-accepted] Done!
You:    [merges without reading] ...later: it prints the wrong version, in a second
        hardcoded place, and there's no test to catch it.
```
