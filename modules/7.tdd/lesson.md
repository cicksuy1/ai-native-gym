...
*Transcript 7-1: red → commit → green, tests untouched.*

```
You:    On the sandbox, NoteStore.add() should take a priority (low/normal/high,
        default normal) and reject anything else. FIRST write tests/test_priority.py
        for: default is normal, an explicit high sticks, and an invalid priority
        raises NoteError. Do NOT implement yet.

Claude: [writes sandbox/tests/test_priority.py with 3 cases, runs
        `python -m unittest`] All 3 fail (add() takes no priority arg).
        ✅ red, as expected.

You:    Good — these tests look right. Commit them.
Claude: [commits "test: priority cases"]
You:    Now implement the priority field to pass them. Do not edit the test file.

Claude: [edits notes.py, reruns] 2/3 pass. The invalid-priority case fails: a
        bad value was stored instead of raising. [adds the validation, reruns]
        3/3 green. ✅
You:    [diff touches only notes.py, tests unchanged] Approved.
```
...
*Anti-pattern 7-2: tests after, or tests editable.*

```
You:    add a priority to add() and some tests for it

Claude: [writes the priority field AND tests together; the tests happen to assert
         exactly what the (buggy) code does — including silently storing a bad
         priority instead of rejecting it]
        Done! All tests pass. ✅
