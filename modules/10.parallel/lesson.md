...
*Transcript 10-1: two independent sandbox chores, each isolated.*

```
You:    Two unrelated chores on the sandbox: (a) add a `count` subcommand to
        cli.py, (b) add a module docstring to notes.py. Different files, no
        ordering — run them in parallel, each in its own worktree.

        [claude --worktree / git worktree add for each]

Claude: [worktree-1 on branch chore-count: edits cli.py]
        [worktree-2 on branch chore-docstring: edits notes.py]
        Two branches, disjoint commits, zero shared files, zero conflicts.

You:    [reviews both branches independently, verifies each, merges each on its
         own merits — both merge clean]
```

The two chores were genuinely independent (disjoint files), so parallelism was a real speedup — and
because each ran in its own worktree, there was never a moment where two agents fought over a file.

*Anti-pattern 10-2: parallel on coupled work, no isolation.*

```
You:    build the persistence feature on the sandbox — spin up 3 agents to go faster

Claude: [agent-1 edits notes.py, agent-2 ALSO edits notes.py, agent-3 rewrites the
         storage format they both depend on — all in the same working dir]
        [merge conflicts mid-flight; agent-2 overwrites agent-1; the format
         change breaks both]
...
mechanisms and how agent view auto-creates worktrees. Try **`claude --worktree`** once on a small
sandbox chore (e.g. the `count` subcommand) to feel the isolated checkout. Then read the Open-Debates note in `RESEARCH.md` on when
