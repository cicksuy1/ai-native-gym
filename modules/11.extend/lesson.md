...
*Transcript 11-1: the right mechanism for each need (on the sandbox).*

```
Need: "the sandbox suite keeps drifting red after edits."
You:  [that needs a GUARANTEE] → PostToolUse hook wiring run-tests.sh into
      .claude/settings.json so the suite runs after every edit. Deterministic;
      the model can't forget it.

Need: "I keep re-explaining the add-a-note-then-verify routine."
You:  [that's a reusable WORKFLOW] → a skill like add-and-verify that loads on
      demand. ~40 tokens until used; full body loads when invoked.
...
*Anti-pattern 11-2: wrong mechanism, hidden cost.*

```
You:  [adds "ALWAYS run the tests after editing" as a line in a bloated CLAUDE.md]
      [also installs 5 MCP servers "to be safe"]

Result: the agent edits and skips the tests anyway (CLAUDE.md is context, not
        enforcement — and a long file gets ignored), and 55k tokens are gone
...
hook to hard-block" line. Then look at the sandbox's own `.claude/`: the ready-to-wire
`hooks/run-tests.sh` (a guarantee waiting for a `settings.json` entry) and the `skills/add-and-verify`
example (a workflow). Finally check your own setup: how many MCP servers are loaded, and roughly what
context do they cost before you've said anything?
