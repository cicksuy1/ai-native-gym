...
*Transcript 2-1: a developer resets between tasks and steers a compaction instead of drowning.*

```
You:    [just finished a long exploration of how find/complete work in notes.py]
        /clear
        Now: improve the empty-list message in sandbox/cli.py — what `list`
        prints when there are no notes. Read cli.py for the current wording.

Claude: [fresh window — only cli.py in context, none of the notes.py exploration]
        [edits just the empty-list branch in cli.py]

        ...later, deep in a longer task, the window is filling...
You:    /compact Focus on the cli.py message change and the test result.
Claude: [summary keeps the edit's state + the check; drops resolved tangents]
```

The `/clear` meant the message task started on a clean desk — none of the earlier `notes.py`
spelunking was sitting there raising the noise floor. And the *focused* `/compact` kept the thread
alive without keeping the clutter.

*Anti-pattern 2-2: one infinite conversation.*

```
You:    [50 messages deep — explored notes.py, then edited cli.py, then poked at
         the tests, then a docstring tweak, all in one session, never cleared]
...
Then look at the sandbox's `sandbox/CLAUDE.md` (or your *own* `~/.claude/CLAUDE.md`) and count the
lines — is every line earning its rent in *every* session, or does some of it belong in an on-demand
skill?
