...
*Transcript 3-1: interview → SPEC.md → fresh session.*

```
You:    [plan mode] We need to add webhook notifications. Before any code,
        interview me on the requirements — ask about events, delivery, retries,
        and what's out of scope. Then write SPEC.md.

Claude: [uses AskUserQuestion]
        - Which events fire a webhook? (order.created, order.shipped, …)
        - Retry policy on a failed delivery? (exponential backoff, max 5?)
        - Out of scope for v1? (signing? batching?)
You:    [answers — naming retries=5 backoff, signing deferred to v2]

Claude: [writes SPEC.md: scope, events table, payload schema, retry rules,
         a "never / ask-first / always" boundaries block, and a verify section]

You:    [reads SPEC.md, fixes one wrong assumption] Approved.
        /clear
        Implement SPEC.md, one section at a time. Start with the event emitter.
```

The interview dragged the implicit (retries? signing? scope?) into the open *before* a line was
written. The `/clear` + "implement SPEC.md" started execution on a clean desk holding only the
contract. The boundaries block told the agent where the rails are.

*Anti-pattern 3-2: the wish.*

```
You:    add webhook notifications to the app

Claude: [guesses: fires on every DB write, no retries, invents a payload shape,
         silently logs failures] Done!
