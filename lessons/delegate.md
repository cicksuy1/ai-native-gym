...
*Transcript 7-1: a scoped subagent returns a destination, not a journey.*

```
You:    [main session, mid-feature — don't want to flood context with a search]
        Spawn a subagent:
        OBJECTIVE: find every call site of the deprecated getUser() and whether
          each passes a userId or a full user object.
        FORMAT: a table — file:line | arg type | notes.
        TOOLS: grep/read only. Read-only.
        BOUNDARIES: do not edit anything; do not follow into node_modules.

Subagent: [greps the whole repo in ITS window, reads 14 call sites]
          Returns a 14-row table. No edits.

You:    [your main context gained a 14-row table, not 14 files of grep output]
        Good — now I can plan the migration.
```
...
*Anti-pattern 7-2: the vague flood.*

```
You:    look into the getUser usages and tell me about them

Subagent: [unclear what "about them" means — dumps full file contents of every
           match, plus its reasoning, back into your main context]
You:    [main window now bloated with raw grep output; I have to re-read it all
