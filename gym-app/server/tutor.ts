...
// conductor conversation (the ai-gym skill), scoped to a module: every module
...
export function notifyModuleComplete(slug: string): void {
  if (!host || host.state === "dead") return;
  host.queue.push(
    `Module ${slug} is complete — run the ai-coach close-out now: ask the learner the short ` +
      `reflection, refresh progress/STRATEGY.local.md, and write your ai-memory notes block to ` +
