...
// for the read-only verification commands ai-verify needs (git read subcommands,
...
// from the *untrusted* transcript the verifier reads. So no `&&`: ai-verify's
...
// subcommands + unittest), which is the accepted tradeoff so ai-verify works.
const BASH_ALLOWLIST: readonly RegExp[] = [
  ...
  // the module check ai-verify re-runs (incl. `discover -s sandbox -t sandbox`).
